...
def generate_test_data(num=10):
    ...
    for i in range(num):
        ...
        name = f"{random.choice([1, 2, 3, 4, ])}"
        pointId = '610191'
        test_data.append((name, pointId))
    return test_data
...
def insert_data_to_pg():
    ...
    try:
        ...
        insert_sql = """
            INSERT INTO ods_event_data (user_id, point_id,created_at)
            VALUES (%s, %s,CURRENT_DATE - INTERVAL '15 day');  -- id/create_at自动填充，update_at/deleted_at默认NULL
        """
        # 执行批量插入
