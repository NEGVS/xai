...
def insert_data_to_pg():
    ...
    try:
        ...
        insert_sql = """
            INSERT INTO srzp_service.user_position_tracking_log (user_id, email,created_at)
            VALUES (%s, %s,CURRENT_DATE - INTERVAL '1 day');  -- id/create_at自动填充，update_at/deleted_at默认NULL
        """
        # 执行批量插入
